...
def generate_save_path(subdirectory: str, file_prefix: str) -> str:
    """
    生成带时间戳的保存路径
    
    Args:
        subdirectory: 保存子目录，如 "dialogue", "script", "outline"
        file_prefix: 文件前缀，如 "dialogue", "script", "outline"
    
    Returns:
        完整的保存路径字符串
    """
    project_root = get_project_root()
    save_dir = project_root / "public" / "sources" / subdirectory
    save_dir.mkdir(parents=True, exist_ok=True)
    
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    save_path = save_dir / f"{file_prefix}_{timestamp}.json"
